...
- Quickstates serialize CPU contexts, allocated guest memory pages, allocator maps, page CRCs, guarded disk replacement, readback-validated saves, a retained backup slot, a durable load-undo slot, and checked named metadata sections for timing, kernel objects, Sysmem block UID tables, SceFiber host CPU contexts, IO/VFS, display, audio, Audiodec decoder handles, AVPlayer/movie, Jpeg/MJPEG decoder state, Videodec/H.264 decoder handles, and NGS host state. The Windows gate now restores when the manifest says mandatory layers are present, recreating saved thread identities, Sysmem memblock handles and counters, SceFiber host pointers inside guest fiber structs, IO file handles, directory handles, FIOS overlays, deferred waits, display/audio scalars, rebuilt audio ports, Audiodec AT9/MP3/AAC decoder tables, AVPlayer runtime scalars, Jpeg/MJPEG decoder objects, Videodec decoder shells, and NGS state. Current UPPERS/DOA canaries exercise open file handles and nonzero SceFiber snapshots; media-focused canaries have separately exercised AAC decoder shells, while directory/overlay restore is versioned and fail-closed until a nonzero real-game case is captured. Durable kernel restore also remaps timer/rwlock identities, clears boot-time simple-event/timer/rwlock queues when the saved state has no live waiters, and routes timed deferred waits plus timer event wakeups through a kernel-owned scheduler with generation guards. Extra boot threads release their old stack/TLS ownership before allocator maps are replaced, saved stack/TLS allocation maps are normalized, stale display callbacks are generation-guarded, and a Windows quickstate harness checks UPPERS/DOA canaries before Android claims. Remaining unsafe paths still refuse the load with a marker instead of resuming from guessed state.
