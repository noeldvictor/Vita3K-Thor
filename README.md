...
- Fast-forward presets: pick 2x, 3x, or 4x from the pause OSD, with the current speed shown live and the multiplier applied immediately if fast-forward is already running. Fast-forward avoids high-pitched chipmunk audio by using pitch-preserving tempo filtering when available, or normal-pitch buffer skipping when that filter is missing.
- Fast-forward is visible while it runs: the on-screen badge reads `60 FPS  2x` alongside the performance overlay, and shows the multiplier on its own even when that overlay is switched off, so the emulator is never quietly left at 3x. The FPS overlay itself has a toggle in the same OSD panel.
- The pause OSD is controller-navigable: D-pad moves, A activates, B goes back.
- Per-game quickstate workbench: capture and restore a disk-backed slot 0 for the current game from the OSD or shortcuts. Loads create a durable undo slot first, so the OSD can restore the state you were in right before the last load. Windows canary testing now reaches PPSSPP-style app-restart durability for UPPERS and DOA Venus; Android/Thor proof is tracked separately before claiming handheld parity.
...
- Save states have a Windows PPSSPP-style durability gate for the current canary set. UPPERS and DOA Venus pass restart durable load, save-again, same-session load, undo-load, restart-again load, five-cycle soak, corrupt-primary `.bak` fallback, custom state root, and compression checks. New saves use temp-file replacement, CRC/readback validation, a retained backup slot, and a separate pre-load undo slot. That means Windows canary quickstates are expected to survive app restarts and recover from a damaged primary state file. This is not yet a blanket all-games claim: wider game coverage, callback edge cases, renderer-resource gaps, and Android/Thor proof still need their own green gates and should fail closed instead of guessing.
...
- Quickstates serialize CPU contexts, allocated guest memory pages, allocator maps, page CRCs, guarded disk replacement, readback-validated saves, a retained backup slot, a durable load-undo slot, and checked named metadata sections for timing, kernel objects, Sysmem block UID tables, SceFiber host CPU contexts, SceSharedFb framebuffer metadata, IO/VFS, display, audio, Audiodec decoder handles, AVPlayer/movie, Jpeg/MJPEG decoder state, Videodec/H.264 decoder handles, NGS host state, and host-service state. The Windows gate now restores when the manifest says mandatory layers are present, recreating saved thread identities, Sysmem memblock handles and counters, SceFiber host pointers inside guest fiber structs, shared-framebuffer service metadata, IO file handles, directory handles, FIOS overlays, deferred waits, display/audio scalars, rebuilt audio ports, GXM render targets/program host objects, Audiodec AT9/MP3/AAC decoder tables, AVPlayer runtime scalars, Jpeg/MJPEG decoder objects, Videodec decoder shells, NP/Net/NetCtl state, and NGS state. Current UPPERS/DOA canaries exercise open file handles, nonzero SceFiber snapshots, DOA NP/NetCtl state, AAC/Videodec media state, and deferred lightweight-mutex waits; SharedFb is covered as mandatory zero-state metadata and still needs a nonzero real-game proof. Directory/overlay restore is versioned and fail-closed until a nonzero real-game case is captured. Durable kernel restore also remaps timer/rwlock identities, clears boot-time simple-event/timer/rwlock queues when the saved state has no live waiters, rebuilds restorable mutex/lwmutex wait queues from saved deferred metadata, and routes timed deferred waits plus timer event wakeups through a kernel-owned scheduler with generation guards. Extra boot threads release their old stack/TLS ownership before allocator maps are replaced, saved stack/TLS allocation maps are normalized, stale display callbacks are generation-guarded, and a Windows quickstate harness checks UPPERS/DOA canaries before Android claims. Remaining unsafe paths still refuse the load with a marker instead of resuming from guessed state.
