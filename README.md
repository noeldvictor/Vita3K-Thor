...
- Save states are an active reliability target. The goal is PPSSPP-style durable per-game save/load, but current builds only write guarded disk-backed captures plus restore-readiness metadata. Timing and matched-thread metadata now have parsed restore helpers, but loading is still intentionally refused until Windows proves kernel wait objects, host syscall state, IO/VFS handles, GPU/display, renderer resources, audio, and AVPlayer/movie paths can resume without crashes or corruption.
...
- Quickstates currently serialize CPU contexts, allocated guest memory pages, allocator maps, page CRCs, guarded disk replacement, and checked named metadata sections for timing, kernel objects, IO/VFS, display, and audio. Timing and matched-thread metadata sections are parsed and have restore code ready behind the gate. The slot marker records restore-readiness with section names, kernel object counts, IO/VFS counts, AVPlayer detection, and missing serializer coverage. Restore is disabled in the Windows stability gate until the remaining layers are implemented and verified.
