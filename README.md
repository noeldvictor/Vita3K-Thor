...
- Save states are an active reliability target. Windows durable slot-0 save/load now passes restart, save-again, same-session load, and restart-again loops for UPPERS and DOA Venus. It is still experimental rather than a broad PPSSPP-level guarantee: unsafe movie/AVPlayer captures, unsupported host wait continuations, callback edge cases, renderer-resource gaps, and Android/Thor proof still need more hardening and should fail closed instead of guessing.
...
- Quickstates serialize CPU contexts, allocated guest memory pages, allocator maps, page CRCs, guarded disk replacement, and checked named metadata sections for timing, kernel objects, IO/VFS, display, audio, and NGS host state. The Windows gate now restores when the manifest says mandatory layers are present, recreating saved thread identities, IO handles, deferred waits, display/audio scalars, and NGS state. Remaining unsafe paths still refuse the load with a marker instead of resuming from guessed state.
