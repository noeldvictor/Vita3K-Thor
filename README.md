...
- Save states are an active reliability target. The goal is PPSSPP-style durable per-game save/load, but current builds still fail closed by default. Captures now include guarded disk files, timing, matched-thread metadata, sync primitive scalar state, matched open-file offsets, and restore-readiness metadata. Loading remains intentionally refused until Windows proves thread lifecycle, unsafe wait queues, host syscall state, missing/recreated IO/VFS handles, GPU/display, renderer resources, audio, and AVPlayer/movie paths can resume without crashes or corruption.
...
- Quickstates currently serialize CPU contexts, allocated guest memory pages, allocator maps, page CRCs, guarded disk replacement, and checked named metadata sections for timing, kernel objects, IO/VFS, display, and audio. Timing, matched-thread metadata, sync primitive scalar state, and matched open-file positions are parsed and have restore code ready behind the gate. The slot marker records restore-readiness with section names, kernel object counts, sync wait queue counts, IO/VFS counts, AVPlayer detection, and missing serializer coverage. Restore is disabled in the Windows stability gate until the remaining layers are implemented and verified.
