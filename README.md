...
- Save states are an active reliability target. Windows durable slot-0 save/load now passes restart, save-again, same-session load, and restart-again loops for UPPERS and DOA Venus. New saves are written to a temp file, read back through CRC validation before replacing the live slot, and keep the previous slot as a fallback if the primary file is corrupt. It is still experimental rather than a broad PPSSPP-level guarantee: callback edge cases, renderer-resource gaps, wider game coverage, and Android/Thor proof still need more hardening and should fail closed instead of guessing.
...
- Quickstates serialize CPU contexts, allocated guest memory pages, allocator maps, page CRCs, guarded disk replacement, readback-validated saves, a retained backup slot, and checked named metadata sections for timing, kernel objects, IO/VFS, display, audio, AVPlayer/movie, and NGS host state. The Windows gate now restores when the manifest says mandatory layers are present, recreating saved thread identities, IO handles, deferred waits, display/audio scalars, rebuilt audio ports, AVPlayer runtime scalars, and NGS state. Extra boot threads release their old stack/TLS ownership before allocator maps are replaced, saved stack/TLS allocation maps are normalized, stale display callbacks are generation-guarded, and a Windows quickstate harness checks UPPERS/DOA canaries before Android claims. Remaining unsafe paths still refuse the load with a marker instead of resuming from guessed state.
