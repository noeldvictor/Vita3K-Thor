...
- Per-game quickstate workbench: capture and restore a disk-backed slot 0 for the current game from the OSD or shortcuts. Each capture writes a restore-readiness note and embedded subsystem sections that say which emulator layers are captured and which ones still block safe loads.
...
- Save states are an active reliability target. Windows durable slot-0 save/load now passes restart, save-again, same-session load, and restart-again loops for UPPERS and DOA Venus, including movie/AVPlayer-window captures and audio-port rebuilds. It is still experimental rather than a broad PPSSPP-level guarantee: unsupported host wait continuations, callback edge cases, renderer-resource gaps, and Android/Thor proof still need more hardening and should fail closed instead of guessing.
...
- Quickstates serialize CPU contexts, allocated guest memory pages, allocator maps, page CRCs, guarded disk replacement, and checked named metadata sections for timing, kernel objects, IO/VFS, display, audio, AVPlayer/movie, and NGS host state. The Windows gate now restores when the manifest says mandatory layers are present, recreating saved thread identities, IO handles, deferred waits, display/audio scalars, rebuilt audio ports, AVPlayer runtime scalars, and NGS state. Extra boot threads release their old stack/TLS ownership before allocator maps are replaced, and stale display callbacks are generation-guarded. Remaining unsafe paths still refuse the load with a marker instead of resuming from guessed state.
