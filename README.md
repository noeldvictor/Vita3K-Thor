...
- Save states are an active reliability target. The goal is PPSSPP-style durable per-game save/load, but current builds only write guarded disk-backed captures plus restore-readiness metadata. Timing, matched-thread metadata, and matched open-file offsets now have parsed restore helpers, but loading is still intentionally refused until Windows proves kernel wait objects, host syscall state, missing/recreated IO/VFS handles, GPU/display, renderer resources, audio, and AVPlayer/movie paths can resume without crashes or corruption.
