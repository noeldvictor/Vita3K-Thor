...
- Save states are an active reliability target. Windows durable slot-0 save/load now passes a two-cycle restart, save-again, same-session load, undo-load, and restart-again canary for UPPERS and DOA Venus. New saves are written to a temp file, read back through CRC validation before replacing the live slot, keep the previous slot as a fallback if the primary file is corrupt, and write a separate pre-load undo slot before restore. The Windows harness now also proves corrupt-primary fallback by damaging `slot0.thorstate`, loading `.bak`, and restoring the original test file afterward; it also checks custom state roots and compression levels. Timed kernel waits and timer event wakeups now use one kernel-owned scheduler with stale-job generation guards instead of loose detached timeout threads. Current Windows canaries include real SceFiber host-context rebuilds, mandatory SceSharedFb restore metadata, GXM render-target host rebuilds after process restart, NP/NetCtl state for DOA, live AAC Audiodec decoder handles, movie Videodec handles, Jpeg/MJPEG host state, and deferred lightweight-mutex wait rebuilds. It is still experimental rather than a broad PPSSPP-level guarantee: callback edge cases, renderer-resource gaps, wider game coverage, and Android/Thor proof still need more hardening and should fail closed instead of guessing.
