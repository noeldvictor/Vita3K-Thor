...
- Save states are an active reliability target. The goal is PPSSPP-style durable per-game save/load, but current builds still fail closed by default. Captures now include guarded disk files, timing, matched-thread metadata, sync primitive scalar state, matched open-file offsets, display/audio scalar state, and restore-readiness metadata. Loading remains intentionally refused until Windows proves thread lifecycle, unsafe wait queues, host syscall state, missing/recreated IO/VFS handles, renderer resources, vblank callback/wait edge cases, and AVPlayer/movie paths can resume without crashes or corruption.
