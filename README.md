...
- Save states are an active reliability target. The goal is PPSSPP-style durable per-game save/load, but current builds still fail closed by default. Captures now include guarded disk files, timing, matched-thread metadata with pre-pause thread status, sync primitive scalar state, host-backed and memory-backed open-file handle/offset recreation, display/audio scalar state, display vblank wait metadata, and restore-readiness metadata. Loading remains intentionally refused until Windows proves full thread lifecycle, unsafe wait queues, host syscall state, directory/overlay IO handles, renderer resources, vblank callback edge cases, and AVPlayer/movie paths can resume without crashes or corruption.
...
- Quickstates currently serialize CPU contexts, allocated guest memory pages, allocator maps, page CRCs, guarded disk replacement, and checked named metadata sections for timing, kernel objects, IO/VFS, display, and audio. Timing, matched-thread metadata with original pre-pause run/wait/suspend status, sync primitive scalar state, open-file handle/offset state, display/audio scalar state, display vblank wait metadata, and matched open-file positions are parsed and have restore code ready behind the gate. The slot marker records restore-readiness with section names, kernel object counts, sync wait queue counts, display wait/callback counts, IO/VFS counts, AVPlayer detection, and missing serializer coverage. Restore is disabled in the Windows stability gate until the remaining layers are implemented and verified.
