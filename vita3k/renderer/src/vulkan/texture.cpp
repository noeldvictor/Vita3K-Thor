...
void sync_texture(VKContext &context, MemState &mem, std::size_t index, SceGxmTexture texture, const Config &config) {
    ...
    // The layout is part of the descriptor, so a view that comes back in a
    // different layout needs the descriptor rewritten even though it compares
    // equal on sampler and view.
