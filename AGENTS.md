...
- SDL fast-forward audio uses FFmpeg `atempo` for pitch-preserving tempo changes when available; if the filter cannot initialize, fall back to SDL frequency-ratio speed-up rather than callback-local grain skipping, which caused choppy audio.
