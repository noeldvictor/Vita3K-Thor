...
- DOA Venus title corruption had two historical symptoms: native BCn sampling could produce magenta terrain on Adreno/Turnip, and a later stale/slab flicker was traced to DoubleBuffer mapped data that crossed the guarded mapping boundary. The current fix is to include the guard page in mapped-memory completeness checks and sync that boundary-crossing data; after commit `25b34732`, an AYN Thor 80-frame burst plus a 45-second screenrecord showed clean title-loop rendering. Keep CPU BCn fallback for Adreno/Turnip defaults, but do not keep chasing the stale "BCn alone causes title flicker" theory unless new evidence reproduces it on a current build.
...
- Android live texture experiments support `debug.vita3k.force_bcn_decompress=1` to disable native BCn/DXT Vulkan sampling and force Vita3K's CPU decompression path on the next renderer startup. Current Thor builds default Adreno/Turnip to the CPU BCn path because DOA Venus burst evidence proved native BCn can produce magenta terrain; use `debug.vita3k.allow_native_bcn=1` only as a regression/diagnostic opt-out.
