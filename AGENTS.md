...
- `fast-forward-speed-percent` defaults to 200 and is clamped from 101 to 1000 when toggled. Fast-forward must update both display/vblank pacing and kernel pacing; keep `emuenv.display.speed_percent` and `emuenv.kernel.speed_percent` in sync so vblank waits, `sceKernelDelayThread`, kernel timers, and wait timeouts do not stay at real-time speed.
