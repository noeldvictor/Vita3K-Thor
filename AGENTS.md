...
- DOA Venus on Thor currently has at least two separable renderer symptoms: default/native BCn sampling can produce magenta terrain or sea on Adreno/Turnip, while `debug.vita3k.force_bcn_decompress=1` removes the magenta but leaves intermittent slab/flicker. Do not use the old `debug.vita3k.render_disable_depth_bias_fhash=564cd0` experiment as a fix; bursts showed it did not materially solve the remaining flicker.
